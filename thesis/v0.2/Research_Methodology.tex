...
% 如果帧长度（即两次计划间隔）大于计划长度，则在计划结束后，agent执行默认行为：原地不动。
...
% 计划结束后执行默认行为是为了将planning horizon和frame length解耦，因为计划的作用就是为agent寻找未来的可用点，若没有默认行为，则frame length必须不能大于planning horizon.
...
% 另外，由于默认行为不是像计划一样遵循约束生成的，为了使其满足无碰撞约束，默认行为必须在没有其他agent计划的时域中进行。
...
% 为了达到这一条件，引入了如下约束: 每次计划时，agent必须生成同样长度的计划。
...
% 这样保证了每个计划的末尾都是全体agent horizon的最前端，在此时间点后没有其他计划，也就可以忽略无碰撞约束而执行默认行为（原地待机）。
